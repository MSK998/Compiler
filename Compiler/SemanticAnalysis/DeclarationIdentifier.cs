    ...
    public class DeclarationIdentifier
    {
        ...
        private void PerformIdentification(IAbstractSyntaxTreeNode node)
        {
            if (node is null)
            {
                ...
                Reporter.ReportError($"Tried to perform identification on a null tree node {node.Position.ToString()}");
            }
            else if (node is ErrorNode)
            {
                ...
                //Reporter.ReportError($"Tried to perform identification on an error tree node {node.Position.ToString()}");
            }
            ...
            else
            {
                ...
                { 
                    ...
                    Reporter.ReportError($"Couldnt find the functuon {functionName} when doing identification {node.Position.ToString()}");
                }
                else
